$(window).on("load", function () {
    $(".tf-swiper").each(function (index, element) {
        ...
        // Destroy existing Swiper instance if it exists to prevent double-initialization
        if ($this[0] && $this[0].swiper) {
        ...
        // Only add grid config if rows > 1 (fade effect doesn't work well with grid)
        if (gridRows > 1 && effect !== "fade") {
            swiperConfig.grid = { rows: gridRows, fill: "row" };
            ...
            // Standard breakpoints without grid for fade effect or single row
            swiperConfig.breakpoints = {
                575: {
                    slidesPerView: mobileSm,
                    spaceBetween: spacing,
        ...
        // Add fade effect config for crossFade
        if (effect === "fade") {
            swiperConfig.fadeEffect = { crossFade: true };
        }

        var swiperT = new Swiper($this[0], swiperConfig);
        $(".swiper-button")
            ...
            if (
                ...
                if (!swiperMb) {
                    ...
                    swiperMb = new Swiper($this[0], {
                        ...
                        navigation: {
                            nextEl: $this.find(".nav-next-mb")[0],
                            prevEl: $this.find(".nav-prev-mb")[0],
                        },
